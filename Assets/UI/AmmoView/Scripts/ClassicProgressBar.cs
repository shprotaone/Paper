...
public class ClassicProgressBar : MonoBehaviour {
	...
	public void Awake() {
		...
		for (int i = 0; i < m_NumberOfSegments; i++) {
			...
			segmentImage.transform.GetComponent<RectTransform> ().sizeDelta = new Vector2(m_SizeOfSegment, segmentImage.GetComponent<RectTransform> ().sizeDelta.y);
			segmentImage.transform.position += (Vector3.right * i * m_SizeOfSegment) - (Vector3.right * (m_RectTransform.sizeDelta.x)/ 2) + (Vector3.right * i * m_SizeOfNotch);
			...
			segmentFillImage.transform.GetComponent<RectTransform> ().sizeDelta = new Vector2(m_SizeOfSegment, segmentImage.GetComponent<RectTransform> ().sizeDelta.y);
		}
	}

	public void Update() {
		for (int i = 0; i < m_NumberOfSegments; i++) {
			m_ProgressToFill [i].fillAmount = m_FillAmount - i;
		}
	}
